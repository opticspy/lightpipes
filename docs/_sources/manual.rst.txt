...
All the calculations must start with the :func:`~LightPipes.Begin` command.
...
Units are included in the LightPipes package. 
...
A two dimensional array will be defined containing the complex numbers with
the real and imaginary parts equal to one and zero respectively.
...
The simplest component to model is an aperture.
There are three different types of apertures:
...
Where R=radius, xs and ys are the shift in x and y direction respectively,
...
:func:`~LightPipes.CircScreen` (inversion of the circular aperture),
:func:`~LightPipes.RectScreen`, (inversion of the rectangular aperture)
and :func:`~LightPipes.GaussScreen` (inversion of the gauss aperture).
...
All kinds of combinations of circular, rectangular and Gaussian apertures
and screens can be made:
...
In figure 1 and 2 the intensity of the field is calculated after aperturing
‘Field’ with the :func:`~LightPipes.Intensity` command using option 0,
...
"matplotlib" which can be installed by typing at the python prompt:
...
With the command :func:`~LightPipes.Interpol` you can reduce the
...
package which can be installed by typing at the python prompt:
...
There are four different possibilities for modelling the light propagation
in LightPipes.

FFT propagation (spectral method).
----------------------------------

Let us consider the wave function :math:`U` in two planes:
:math:`U(x,y,0`) and :math:`U(x,y,z)`. Suppose then that 
:math:`U(x,y,z)` is the result of propagation of
:math:`U(x,y,0)` to the distance :math:`z` , 
with the Fourier transforms of these two (initial and propagated )
wave functions 
given by :math:`A(\alpha,\beta,0)` and :math:`A(\alpha,\beta,z)`
correspondingly. 
...
:math:`H( \alpha,\beta,z)` , given by [#f1]_ [#f2]_ :
...
Expressions :eq:`1`, :eq:`2`, and :eq:`3` provide a symmetrical relation
between the initial and diffracted wave 
functions in the Fresnel approximation. Applied in the order 
:eq:`2` :math:`\rightarrow` :eq:`1` :math:`\rightarrow` :eq:`3` 
...
:eq:`1`, :eq:`2` and :eq:`3` with operators 
:math:`L^+` and :math:`L^-` respectively. The described algorithm can be
implemented numerically using Fast Fourier Transform (FFT) [#f2]_ [#f3]_ on
...
the frequency domain should be applied, see [#f2]_ [#f3]_ for 
extensive analysis of these computational aspects. The simplest and fastest
LightPipes command for propagation is :func:`~LightPipes.Forvard`. (The ‘v’ is a type error made on purpose!) 
...
We see the diffraction effects, the intensity distribution is not uniform anymore.
...
to a distance comparable with :math:`D^2/\lambda` where :math:`D` is the diameter
(or a characteristic size) of 
the beam, and :math:`\lambda` is the wavelength. To propagate the beam to
...
beams reach the region border sooner. 
...
may be supplied to :func:`~LightPipes.Forvard`. It means that the program will
...
Direct integration as a convolution: FFT approach.
--------------------------------------------------
...
Replacing the functions U(x) and U(x1) with step functions Uj and Um, defined in
the sampling points of the grid with j=0…N, and m=0…N we convert the integral (5.5) to the form:
...
where: :math:`K_{m0}`, :math:`K_{mj}` and :math:`K_{mN}` are analytical expressed
with the help of Fresnel 
integrals, depending only onto the difference of indices. The summations
:math:`\sum_{j=1}^{N-1} U_j K_{mj}` can 
easily be calculated for all indices m as one convolution with the help of FFT.

The command :func:`~LightPipes.Fresnel` implements this algorithm using the
trapezoidal rule. It is almost as fast 
as :func:`~LightPipes.Forvard` (from 2 to 5 times slower), it uses 8 times more
memory than :func:`~LightPipes.Forvard` and it allows 
...
of :func:`~LightPipes.Forvard`. :func:`~LightPipes.Fresnel` does not accept a negative propagation distance. When possible :func:`~LightPipes.Fresnel`
has to be used as the main computational engine within LightPipes.

**Warning:** :func:`~LightPipes.Fresnel` does not produce valid results if the distance of propagation 
is comparable with (or less than) the characteristic size of the aperture 
at which the field is diffracted. In this case :func:`~LightPipes.Forvard` or
:func:`~LightPipes.Steps` should be used.
...
:math:`N^4` , where :math:`N` is the grid sampling. With direct integration
we do not have any reflection at the grid boundary, so the size of the grid can just match the cross section of field distribution. LightPipes include a program :func:`~LightPipes.Forward` realizing direct integration. :func:`~LightPipes.Forward` has the following features:
...
The :func:`~LightPipes.Steps` command has a built-in absorption layer along the grid boundaries 
(to prevent reflections), occupying 10% of grid from each side. :func:`~LightPipes.Steps` is the only command in LightPipes 
allowing for modeling of (three-dimensional) waveguide devices.

Like :func:`~LightPipes.Forvard`, :func:`~LightPipes.Steps` can inversely propagate the field, for example the sequence 
...
:func:`~LightPipes.Steps` implements scalar approximation, it is not applicable for modeling of 
...
In this example the wave front was tilted by :math:`\alpha = 0.1 mrad` in X and Y directions, then propagated to the distance :math:`z = 8 m` , so in the output distribution we observe the transversal shift of the whole intensity distribution by :math:`\alpha z = 0.8 mm` .

Zernike polynomials.
^^^^^^^^^^^^^^^^^^^^

Any aberration in a circle can be decomposed over a sum of Zernike polynomials. Formulas given in [#f7]_ have been directly implemented in LightPipes. The command is called :func:`~LightPipes.Zernike` and accepts four command line arguments:
...
We can uniformly introduce :func:`~LightPipes.Lens` and :func:`~LightPipes.Tilt` with :func:`~LightPipes.Zernike`, the difference is that we pass the amplitude of the aberration to  :func:`~LightPipes.Lens`, :func:`~LightPipes.Tilt` and :func:`~LightPipes.Zernike` accept conventional meters and radians, which are widely in use for the description of optical setups, while :func:`~LightPipes.Zernike` uses the amplitude of the aberration, which frequently has to be derived from the technical description.
...
The bending of the coordinate system for the :func:`~LightPipes.LensFresnel` command is given by 
:math:`f_2`.
...
There are two random filters to introduce a random intensity or a random phase distribution in the field. The commands :func:`~LightPipes.RandomIntensity` and :func:`~LightPipes.RandomPhase` need a seed to initiate the random number generator. The :func:`~LightPipes.RandomPhase` command needs a maximum phase value (in radians). The :func:`~LightPipes.RandomIntensity` command yields a normalised random intensity distribution. The *RandomIntensity* and the :func:`~LightPipes.RandomPhase` commands leave the phase and the intensity unchanged respectively.
...
The :func:`~LightPipes.Gain` command introduces a simple single-layer model of a laser amplifier. The output field is given by:
